...
docker compose build --no-cache soulseek-bulk-downloader
...
mkdir -p logs
docker compose logs soulseek-bulk-downloader > "logs/soulseek-bulk-downloader-$(date +%Y%m%d_%H%M%S).log" 2>&1
echo "✅ Log saved!"
